...
impl<'a> UserManagementApi<'a> {
    ...
    ) -> Result<crate::pagination::Page<UserSessionsListItem>, Error> {
    ...
    /// Returns an async [`futures_util::Stream`] that yields every `UserSessionsListItem`
    /// across all pages, advancing the `after` cursor under the hood.
    ///
    /// ```ignore
    /// use futures_util::TryStreamExt;
    /// let all: Vec<UserSessionsListItem> = self
    ///     .list_sessions_auto_paging(id, params)
    ///     .try_collect()
    ///     .await?;
    /// ```
    pub fn list_sessions_auto_paging(
        ...
        id: impl Into<String>,
        params: ListSessionsParams,
    ) -> impl futures_util::Stream<Item = Result<UserSessionsListItem, Error>> + '_ {
        ...
        crate::pagination::auto_paginate_pages(move |after| {
            ...
            params.after = after;
            async move {
                let page = self.list_sessions(&id, params).await?;
                Ok((page.data, page.list_metadata.after))
            }
        })
    }
    ...
    /// Returns an async [`futures_util::Stream`] that yields every `UserInvite`
    /// across all pages, advancing the `after` cursor under the hood.
    ///
    /// ```ignore
    /// use futures_util::TryStreamExt;
    /// let all: Vec<UserInvite> = self
    ///     .list_invitations_auto_paging(params)
    ///     .try_collect()
    ///     .await?;
    /// ```
    pub fn list_invitations_auto_paging(
        &self,
        params: ListInvitationsParams,
    ) -> impl futures_util::Stream<Item = Result<UserInvite, Error>> + '_ {
        crate::pagination::auto_paginate_pages(move |after| {
            let mut params = params.clone();
            params.after = after;
            async move {
                let page = self.list_invitations(params).await?;
                Ok((page.data, page.list_metadata.after))
            }
        })
    }
